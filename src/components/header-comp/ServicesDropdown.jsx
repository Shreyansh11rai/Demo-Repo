...
import CenterDropdown from "@/components/header-comp/CenterDropdown";
  ...
  return (
    <li className="group">
      ...
      <div className="top-13 invisible absolute left-0 z-50 w-full scale-75 rounded-lg pt-10 opacity-0 transition-all duration-200 group-hover:visible group-hover:scale-100 group-hover:opacity-100">
        <div className={"bg-gradient-to-r from-primaryclr to-slate-500 rounded-2xl md:px-10 md:py-5 shadow-2xl max-w-[70rem] mx-auto"}>
          <div className="grid w-[100%] grid-cols-12 rounded-3xl">
            ...
            <ul className="col-span-4 flex h-[25rem] flex-col gap-3 rounded-s-md border-r-2 border-gray-300 bg-white p-6">
              {data.dropdownData.map((menu, index) => (
                <li 
                key={index + "btn"}
                >
                <Link
            ...
            {/* Center Content */}
            <CenterDropdown data={data.dropdownData} activeList={activeList} />
            {/* image container  */}
            <div className="col-span-4 ml-2 overflow-hidden rounded-lg bg-white h-[25rem] p-4">
              <Image
                height={400}
                width={300}
                src={headerData.dropdownImage}
                alt="Service Image"
